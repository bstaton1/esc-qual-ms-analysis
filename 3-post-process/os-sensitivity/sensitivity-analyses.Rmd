...
subtitle: "_Online Supplement S1 to Staton et al._"
...
col = character(length(ids))
col[str_detect(ids, "0")] = "blue"
col[str_detect(ids, "ASL")] = "red"
lty = rep(1:4, 2)
lwd = c(3, rep(1, length(ids) - 1))
```
...
**Figure 2.3:** Consistency in the estimated value of $S_{\text{MSC}}$ across assumed fecundity data sources and models for (a) 8-inch mesh and (b) 6-inch mesh. Points represent posterior medians and error bars represent the central 50% (thick lines) and 80% (thin lines) posterior credible regions.
...
mp = barplot(x[,zero_mods], beside = T, names.arg = LETTERS[1:6],
             yaxt = "n",
             ylim = range(msy[c("10%", "90%"),"S",,,]) * c(0.95,1.05), col = "white", border = "white", xlab = "Fecundity Dataset", ylab = "SMSC")
segments(mp, lwr1, mp, upr1, col = col[zero_mods])
segments(mp, lwr2, mp, upr2, col = col[zero_mods], lwd = 3)
...
mp = barplot(x[,ASL_mods], beside = T, names.arg = LETTERS[1:6],
             yaxt = "n",
             ylim = range(msy[c("10%", "90%"),"S",,,]) * c(0.95,1.05), col = "white", border = "white", xlab = "Fecundity Dataset", ylab = "SMSC")
segments(mp, lwr1, mp, upr1, col = col[ASL_mods])
segments(mp, lwr2, mp, upr2, col = col[ASL_mods], lwd = 3)
