...
The general expression for finding the brood year index $y$ in which fish returning in year $t$ at the $a^{\mathrm{th}}$ age were spawned is $y=t+n_a-a$, where $n_a$ is the number of ages of return. Our model tracked ages 4, 5, 6, and 7 returns for both sexes, so this calculation was the same for both males and females.
...
**Figure 3.1** Visual representation of the apportionment of brood year recruits to calendar year returns at age. Adapted from Figure 1 in @fleischman-etal-2013.
...
**Figure 4.8:** Variability in the estimated value of $S_{\text{MSC}}$ across data weighting scenarios for (a) 8-inch mesh, (b) 6-inch mesh, and (c) non-selective mesh. Points represent posterior medians and error bars represent the central 50% (thick lines) and 80% (thin lines) posterior credible regions. Blue represents the weighting scenarios using the "scale" ESS method, red represent the weighting scenarios using the "min" method. 
...
# read in the posterior samples and meta data
for (i in 1:length(mods)) {
  post_list[[i]] = post_thin(readRDS(postfiles[i]), keep_iters = 100)
  meta[[i]] = readRDS(metafiles[i])
}
...
where $x_{k,t}$ represents the abundance data (simulated or observed) for fate $k$ in year $t$ and $\hat{x}_{k,t}$ represents the model expectation for MCMC iteration $i$. For the $x_{k,t,i}$, the $i$ index applies only for simulated data as a different set was drawn for each iteration. The metric used to summarize the fit of observed composition data from fate $k$ was the $\chi^2$ statistic:

$$\chi_{k,i}^2 = \sum_t^{n_t} \sum_j^{n_s \cdot n_a} \frac{(x_{k,t,j,i} - \hat{x}_{k,t,j,i})^2}{\hat{x}_{k,t,j,i}}$$
where $x_{k,t,j}$ represents the observed (or simulated) data (counts at age/sex $j$) for year $t$, and $\hat{x}_{k,t,j}$ represents the model expectation for MCMC iteration $i$. For each model and data set, we summarized these fit criteria as the proportion of MCMC iterations in which the observed data fitted more poorly than the simulated data (i.e., a Bayesian $p$-value), with values approaching 0.5 indicating ideal model fit and values  closer to zero or one indicating under- or over-dispersion, respectively [@kery-2010; @gelman-etal-2014].

**Table 5.1**  Bayesian $p$-values by data type (a -- abundance vs. b -- composition) and fate (commercial harvest, escapement, or subsistence harvest). Values close to 0.5 indicate good agreement between model and data, values closer to zero indicate data are under-dispersed relative to the model, and values closer to one indicated data are over-dispersed relative to the model.
...
# Section #6: $S_{\text{MAX}}$ instead of $S_{\text{MSC}}$

Our findings in the main text equilibrium analysis illustrate that $S_{\text{MSC}}$ is sensitive to the specific demographic trends, time periods, and size-selectivity used to derive it. However, here we illustrate that we would have made similar conclusions had we used an alternative biological reference point: $S_{\text{MAX}}$. $S_{\text{MAX}}$ represents the total escapement (regardless of sex or age composition) necessary to maximize recruitment. The yield-per-recruit approach we employed is general enough as to allow maximizing another objective value -- for deriving $S_{\text{MSC}}$ in the main text analysis we set the algorithm to maximize total catch but by replacing this with total recruitment, we can find $S_{\text{MAX}}$ instead. Figure 6.1 below shows the same figure as presented in the main text Figure 7, but for $S_{\text{MAX}}$ and $H_{\text{MAX}}$ (harvest at maximum recruitment). Note the overall similarity between this figure and that for $S_{\text{MSC}}$ as shown in Figure 7 of the main text.

**Figure 6.1** Estimated equilibrium values of $S_{\text{MAX}}$ (panels a, c, and e) and $H_{\text{MAX}}$ (panels b, d, and f) using 8-inch (panels a and b), non-selective (panels c and d), and 6-inch (panels e and f) mesh gillnet hear for a subset of models. The time period (symbol type) represents which years were used to calculate the average demographic qualities used in the equilibrium calculations. The percent change from the posterior median for model N-0 is displayed on the secondary $y$-axis for reference. Points are posterior medians, thick lines are the posterior central 50% limits, and thin lines are the posterior central 80% limits.
...
Rmax_plot = function(keep_val = "S", keep_vuln = "mesh8", keep_mods, xticklabs = F, legend = F, letter) {
  ...
  mp = barplot(keep["50%",,keep_mods], yaxt = "n", xaxt = "n", col = "white", border = "white", las = 2,
               beside = T, 
               ylim = range(Rmax[c("10%", "90%"),keep_val,,,keep_mods]) * c(0.85, 1.2),
               xlim = c(0.5, 20.5) 
               # , ylim = c(ylwr, yupr)
  )
  ...
  mtext(side = 3, ifelse(keep_vuln == "mesh8", latex2exp::TeX(paste0(keep_val, "_{MAX}")), ""), line = 0, font = 2)
...
Rmax_plot("S", "mesh8", keep_mods, legend = T, letter = "a")
Rmax_plot("S", "flat", keep_mods, legend = F, letter = "c")
Rmax_plot("S", "mesh6", keep_mods, xticklabs = T, letter = "e"); par(mar = c(0,2.25,0.75,1.75))
