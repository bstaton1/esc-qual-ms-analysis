...
compare_msy = function(v, value = "S", title = T, ylim, legend = F, labels = F) {
  ...
  bxp(bp_early, at = at_early, add = T, xaxt = "n", yaxt = "n", whisklty = 1, staplewex = 0, boxwex = 0.15, boxfill = "white")
  bxp(bp_all, at = at_all, add = T, xaxt = "n", yaxt = "n", whisklty = 1, staplewex = 0, boxwex = 0.15, boxfill = "grey")
  bxp(bp_late, at = at_late, add = T, xaxt = "n", yaxt = "n", whisklty = 1, staplewex = 0, boxwex = 0.15, boxfill = "grey40")
  ...
  if (title) {
    text(x = c(lborder + fline, fline + lline, lline + rborder)/2,
         ...
         labels = c("Fish Count", "Egg Count", "Egg Mass"), cex = 0.8)
  }
  if (legend) {
...
# ppi = 600
# png("out.png", h = 8 * ppi, w = 4 * ppi, res = ppi)
# par(mfrow = c(3,1), mar = c(2,2,2,2))

# dev.off()
# early_msy = msy["50%",value,v,"early",]
# all_msy = msy["50%",value,v,"all",]
# late_msy = msy["50%",value,v,"late",]
# 
# round((late_msy - all_msy)/all_msy, 2) * 100
# round((late_msy - early_msy)/early_msy, 2) * 100
```
...
```{r}
mypar = function() {
  par(mfrow = c(2,1), mar = c(0,3,1,1), oma = c(2,2,2,2), tcl = -0.25, mgp = c(2.2,0.4,0))
}
```
...
mypar()
ylim = c(160,320) * 1000
compare_msy(v = "unr", value = "R", ylim = ylim, legend = T)
compare_msy(v = "res", value = "R", ylim = ylim, labels = T, title = F)
...
mypar()
ylim = c(40,260) * 1000
compare_msy(v = "unr", value = "H", ylim = ylim, legend = T)
compare_msy(v = "res", value = "H", ylim = ylim, labels = T, title = F)
...
mypar()
ylim = c(40,150) * 1000
compare_msy(v = "unr", value = "S", ylim = ylim, legend = T)
compare_msy(v = "res", value = "S", ylim = ylim, labels = T, title = F)
...
*  Solid line = Fish count, no trends
*  Dashed line = Egg count, all trends
*  Dotted line = Egg mass, all trends
...
pp_plot = function(value, desired, time, vuln) {
  ...
  par(xaxs = "i", yaxs = "i")
  plot(sub[,1] ~ Sval, xlim = c(0,200), ylim = c(0,1), type = "n", xlab = "Total Escapement", ylab = "Pr(Criterion Met)")
...
### MSY {.tabset .tabset-fade .tabset-pills}

#### >90% of MSY {.tabset .tabset-fade .tabset-pills}
...
#### >80% of MSY {.tabset .tabset-fade .tabset-pills}
...
#### >70% of MSY {.tabset .tabset-fade .tabset-pills}
