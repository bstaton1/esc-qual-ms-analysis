...
kable(
  ...
  caption = "Summaries of the $\\hat{R}$ convergence diagnostic for all estimated parameters/states. Values close to 1 are ideal, values greater than 1.1 of moderate concern. Summaries were generated across indices of the parameter; for example the max value for $F_{\\text{com},t}$ represents the maximum $\\hat{R}$ value across all years. Parameters that were fixed to have the same value each MCMC iteration have empty cells.", escape = F, row.names = F) %>%
...
diag_plots(post, c("^Vtau$", "^Vsig$", "^Vtha$", "^Vlam$"), show_diags = "never")
```
...
diag_plots(post, "Fcom", show_diags = "never")
```
...
diag_plots(post, "Fsub", show_diags = "never")
```
...
diag_plots(post, "^R[", show_diags = "never")
```
...
diag_plots(post, "b0_sex", show_diags = "never")
```
...
diag_plots(post, "b1_sex", show_diags = "never")
```
...
diag_plots(post, "b0_mat", show_diags = "never")
```
...
diag_plots(post, "b1_mat", show_diags = "never")
```
...
par(oma = c(0,0,2,0)); diag_plots(post, "D_sum", show_diags = "never")
```
...
junk = sapply(sample(1:nrow(V_post), 100), function(i) {
...
legend("bottomright", legend = c("Males", "Females"), pch = 16,
       col = c("Blue", "red"), lty = 1, pt.cex = 1.2, cex = 0.8)
...
v_prior_plot = function(p) {
  samps = post_subset(post, paste0("V", p), matrix = T)
  ...
  box()
  legend("top", legend = c("Prior", "Posterior", "Yukon"), pch = 22,
...
`r if (selectivity) 'The prior on each selectivity parameter ($\\tau$, $\\sigma$, $\\theta$, and $\\lambda$) was normal with mean equal to the estimates presented in Bromaghin ([2005](https://doi.org/10.1016/j.fishres.2005.03.004){target="_blank"}), and standard deviation equal to the standard error presented therein times 10 (purposefully uninformative). The "Yukon" estimates are from Bromaghin ([2005](https://doi.org/10.1016/j.fishres.2005.03.004){target="_blank"}) with unaltered standard error.'`
...
make_table = function(grp) {
  output = tab[group == grp,] %>%
    kable(format = "html", digits = 2, align = "llccccc", escape = F, row.names = F, big.mark = ",") %>%
    kable_styling(full_width = F, bootstrap_options = c("condensed")) %>%
...
samps_early = prep_samples(post, keep_t = early_keep_t, keep_y = early_keep_y, len_trend = meta$len_trend, silent = T)
samps_late = prep_samples(post, keep_t = late_keep_t, keep_y = late_keep_y, len_trend = meta$len_trend, silent = T)
samps_all = prep_samples(post, keep_t = all_keep_t, keep_y = all_keep_y, len_trend = meta$len_trend, silent = T)
