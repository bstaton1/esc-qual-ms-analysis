...
author: "Incorporating demographic information into spawner-recruit analyses alters biological reference points for a western Alaska salmon population"
date: "DOI: [10.1139/cjfas-2020-0478](https://doi.org/10.1139/cjfas-2020-0478)"
output:
  html_document:
    theme: cerulean
    toc: true
...
# directory to knit from: the project root, which is one directory up from here
...
diag_plot_thin = 0.5  # proportion of samples to keep when traceplotting
```
...
math_key = c(
  ...
  "gamma_0[1,1]"    = "$\\gamma_{0,1,1}$",
  "gamma_0[1,2]"    = "$\\gamma_{0,2,1}$",
  "gamma_0[1,3]"    = "$\\gamma_{0,3,1}$",
  "gamma_0[1,4]"    = "$\\gamma_{0,4,1}$",
  "gamma_1[1,1]"    = "$\\gamma_{1,1,1}$",
  "gamma_1[1,2]"    = "$\\gamma_{1,2,1}$",
  "gamma_1[1,3]"    = "$\\gamma_{1,3,1}$",
  "gamma_1[1,4]"    = "$\\gamma_{1,4,1}$",
  "gamma_0[2,1]"    = "$\\gamma_{0,1,2}$",
  "gamma_0[2,2]"    = "$\\gamma_{0,2,2}$",
  "gamma_0[2,3]"    = "$\\gamma_{0,3,2}$",
  "gamma_0[2,4]"    = "$\\gamma_{0,4,2}$",
  "gamma_1[2,1]"    = "$\\gamma_{1,1,2}$",
  "gamma_1[2,2]"    = "$\\gamma_{1,2,2}$",
  "gamma_1[2,3]"    = "$\\gamma_{1,3,2}$",
  "gamma_1[2,4]"    = "$\\gamma_{1,4,2}$",
  "gamma_0"         = "$\\gamma_{0,a,s}$",
  "gamma_1"         = "$\\gamma_{1,a,s}$",
  "delta_0"         = "$\\delta_{0}$",
  "delta_1"         = "$\\delta_{1}$",
  "Vtau"           = "$\\tau$",
  "Vsig"           = "$\\sigma$",
...
if (thin_post) post = post_thin(post, keep_percent = 0.1)
...
`r cntr = up_cntr("Section", cntr, T); create_label("Section", cntr, F, level = 1, sub = "", subb = "", text = paste0("Model Description: ", model_ids[model]), tabset = T)`

`r cntr = up_cntr("Table", cntr, T); create_label("Table", cntr, T, level = 2, sub = "a", subb = "", text = "Attributes", tabset = F)`
...
`r create_label("Table", cntr, T, level = 2, sub = "b", subb = "", text = "Parameter Counts", tabset = F)`

```{r}
param_counts = suppressWarnings(count_params_in_model(age_trend = meta$age_trend, sex_trend = meta$sex_trend))

tab = data.frame(Type = names(param_counts), Count = unname(param_counts))

kable(tab, caption = "Number of estimated parameters/states in this model.") %>%
  kable_styling(full_width = F, bootstrap_options = "condensed") %>%
  row_spec(nrow(tab), bold = T)
```

`r create_label("Table", cntr, T, level = 2, sub = "c", subb = "", text = "WAIC", tabset = F)`

Estimated Watanabe–Akaike information criterion (WAIC) and effective number of parameters (pD) for this model.

```{r}
tab = data.frame(pD = round(meta$WAIC["pD"]), WAIC = round(meta$WAIC["WAIC"], 1))
rownames(tab) = NULL
kable(tab) %>%
  kable_styling(full_width = F, bootstrap_options = "condensed")
```
...
n_adapt = mean(unname(meta$mcmc.info[str_detect(names(meta$mcmc.info), "adapt")]))
...
par(oma = c(0,0,2,0)); diag_plots(post, "alpha", show_diags = "never", keep_percent = diag_plot_thin)
```

`r create_label("Figure", cntr, F, level = 3, sub = "b", subb = "", text = "$\\beta$", tabset = F, mcmc_info)`

```{r beta-diag-plot, eval = mcmc_info}
par(oma = c(0,0,2,0)); diag_plots(post, "beta$", show_diags = "never", keep_percent = diag_plot_thin)
```

`r create_label("Figure", cntr, F, level = 3, sub = "c", subb = "", text = "$\\phi$", tabset = F, mcmc_info)`

```{r phi-diag-plot, eval = mcmc_info}
par(oma = c(0,0,2,0)); diag_plots(post, "phi", show_diags = "never", keep_percent = diag_plot_thin)
```

`r create_label("Figure", cntr, F, level = 3, sub = "d", subb = "", text = "$\\sigma_R$", tabset = F, mcmc_info)`

```{r sigmaR-diag-plot, eval = mcmc_info}
par(oma = c(0,0,2,0)); diag_plots(post, "sigma_R", show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, c("^Vtau$", "^Vsig$", "^Vtha$", "^Vlam$"), show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, "Fcom", show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, "Fsub", show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, "^R[", show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, "delta_0", show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, "delta_1", show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, "gamma_0", show_diags = "never", keep_percent = diag_plot_thin)
```
...
diag_plots(post, "gamma_1", show_diags = "never", keep_percent = diag_plot_thin)
```
...
par(oma = c(0,0,2,0)); diag_plots(post, "D_sum", show_diags = "never", keep_percent = diag_plot_thin)
```
...
par(mfcol = c(4,2), mar = c(1,2,2,1.5), oma = c(3,4,0,0), cex.main = 1.4, cex.axis = 1.4, mgp = c(2,0.4,0), tcl = -0.2)
...
mtext(side = 1, outer = T, line = 1.5, "Calendar Year", cex = 1.2)
mtext(side = 2, outer = T, line = 1.5, "Proportional Age/Sex Composition", cex = 1.2)

```

`r create_label("Figure", cntr, F, level = 4, sub = "a", subb = "2", text = "Commercial", tabset = F, fit)`

`r if (fit) "_Vertical dashed lines represent the year the fishery switched from mostly 8 inch mesh to mostly 6 inch mesh._"`
...
for (as in 1:8) {
  plot(q_fit[,as] ~ years, type = "n", ylim = range(c(q_lwr[,as], q_upr[,as], q_obs[,as]), na.rm = T),
       main = paste("Age", age_v[as], sex[as]), las = 1, xlab = "", ylab = "")
  polygon(x = c(years, rev(years)), y = c(q_lwr[,as], rev(q_upr[,as])), col = "skyblue", border = NA)
  lines(q_fit[,as] ~ years, lwd = 2, col = "blue")
  lines(q_lwr[,as] ~ years, col = "skyblue3")
  lines(q_upr[,as] ~ years, col = "skyblue3")
...
par(mar = c(3,3,1,1), mgp = c(2,0.4,0), tcl = -0.20)
...
```{r esc-ess, eval = fit}
par(xaxs = "i", yaxs = "i", mar = c(3,3,1,1), mgp = c(2,0.4,0), tcl = -0.2, cex.axis = 1.2)
...
axis(side = 1, at = mp[2:(nt+1)], labels = rep("", nt), line = 0, las = 2)
box()
```
...
par(mar = c(3,3,1,1), mgp = c(2,0.4,0), tcl = -0.2)
...
junk = sapply(sample(1:nrow(V_post), min(nrow(V_post), 100)), function(i) {
  lines(V_out[,i] ~ rlm_seq, col = alpha("grey20", 0.2))
})
...
mesh8_fit = apply(v[,,,1], 3, function(x) apply(x, 2, mean))
mesh8_lwr = apply(v[,,,1], 3, function(x) apply(x, 2, function(z) quantile(z, 0.025)))
mesh8_upr = apply(v[,,,1], 3, function(x) apply(x, 2, function(z) quantile(z, 0.975)))
v = array_format(post_summ(post, "v[")[3,])
...
par(mar = c(3,3,1,1), mgp = c(2,0.4,0), tcl = -0.2)
plot(mesh8_fit[,1] ~ xf, ylim = c(0,1), xlim = range(xf, xm),
     type = "n", xaxt = "n", las = 2,
     xlab = "Age", ylab = "Selectivity", cex = 1.2)
segments(xf, mesh8_lwr[,1], xf, mesh8_upr[,1], col = "red")
segments(xm, mesh8_lwr[,2], xm, mesh8_upr[,2], col = "blue")
points(mesh8_fit[,2] ~ xm,  col = "blue", bg = "skyblue", pch = 21, type = "o", cex = 1.2)
points(mesh8_fit[,1] ~ xf,  col = "red", bg = "salmon", pch = 21, type = "o", cex = 1.2)
...
mesh6_fit = apply(v[,,,2], 3, function(x) apply(x, 2, mean))
mesh6_lwr = apply(v[,,,2], 3, function(x) apply(x, 2, function(z) quantile(z, 0.025)))
mesh6_upr = apply(v[,,,2], 3, function(x) apply(x, 2, function(z) quantile(z, 0.975)))
...
par(mar = c(3,3,1,1), mgp = c(2,0.4,0), tcl = -0.2)
plot(mesh6_fit[,1] ~ xf, ylim = c(0,1), xlim = range(xf, xm),
     type = "n", xaxt = "n", las = 2,
     xlab = "Age", ylab = "Selectivity", cex = 1.2)
segments(xf, mesh6_lwr[,1], xf, mesh6_upr[,1], col = "red")
segments(xm, mesh6_lwr[,2], xm, mesh6_upr[,2], col = "blue")
points(mesh6_fit[,2] ~ xm,  col = "blue", bg = "skyblue", pch = 21, type = "o", cex = 1.2)
points(mesh6_fit[,1] ~ xf,  col = "red", bg = "salmon", pch = 21, type = "o", cex = 1.2)
legend("bottomright", legend = c("Males", "Females"), pch = 21,
       col = c("blue", "red"), pt.bg = c("skyblue", "salmon"), lty = 1, pt.cex = 1.2, cex = 0.8)
axis(side = 1, at = 4:7, labels = 4:7)
...
v_prior_plot = function(p) {
  ...
  if (p %in% c("sig", "tha")) {
    source_samps = rtruncnorm(n = nrow(post_samps), a = 0, b = "Inf", mean = source_p[1], sd = source_p[2])
    prior_samps = rtruncnorm(n = nrow(post_samps), a = 0, b = "Inf", mean = prior_p[1], sd = prior_p[2])
  } else {
  ...
  # draw a blank plot
  par(mar = c(3,3,1,1), xaxs = "i", yaxs = "i", mgp = c(2,0.4,0), tcl = -0.2)
  ...
  # KDE: the prior
  with(dens[[2]], polygon(x = c(x, rev(x)), y = c(y, rep(0, length(y))), col = alpha("orange", 0.5), border = NA))
  lines(dens[[2]]$y ~ dens[[2]]$x, col = "black")
  
  # KDE: the posterior
  with(dens[[1]], polygon(x = c(x, rev(x)), y = c(y, rep(0, length(y))), col = alpha("skyblue4", 0.3), border = NA))
  lines(dens[[1]]$y ~ dens[[1]]$x, col = "blue")
  
  # KDE: the source 
  with(dens[[3]], polygon(x = c(x, rev(x)), y = c(y, rep(0, length(y))), col = alpha("salmon4", 0.3), border = NA))
  lines(dens[[3]]$y ~ dens[[3]]$x, col = "red")
  
  box()
  legend("top", legend = c("Prior", "Posterior", capitalize(meta$Vprior)), pch = 22,
         pt.bg = alpha(c("orange", "skyblue4", "salmon4"), c(0.5, 0.3, 0.3)),
...
if (meta$Vprior == "kusko") {
  sel_message = 'The prior on each selectivity parameter ($\\tau$, $\\sigma$, $\\theta$, and $\\lambda$) was normal (or truncated normal to ensure $\\sigma>0$ and $\\theta>0$) with mean equal to the estimates from the Kuskokwim lower river sonar site provided by the Alaska Department of Fish and Game (ADF&G; see Birchfield and Smith [2019](http://www.adfg.alaska.gov/FedAidPDFs/FDS19-27.pdf){target="_blank"} for methods), and standard deviation equal to the standard error provided times 10 (purposefully uninformative). The "Kusko" estimates are those provided by ADF&G with unaltered standard error.'
}
...
pi = array_format(post_summ(post, "pi")[1,])

layout(matrix(1:3, 1, 3), widths = c(1,1,0.2))
par(mar = c(2,2,2,1), oma = c(2,3,0,0), xaxs = "i", yaxs = "i",
    cex.axis = 1.4, cex.main = 1.4, tcl = -0.2, mgp = c(2,0.4,0))
...
by = (min(years) - a_max):(max(years) - a_min)
par(mar = c(3.5,3.5,1,1), mgp = c(2.3, 0.4, 0), tcl = -0.2)
plot(psi[3,] ~ by, type = "l", xlab = "Brood Year", ylab = "Probability of Return-as-Female", las = 1, ylim = c(0.2, 0.5))
polygon(x = c(by, rev(by)), y = c(psi[4,], rev(psi[5,])), col = "skyblue", border = NA)
lines(psi[4,] ~ by, col = "skyblue3")
lines(psi[5,] ~ by, col = "skyblue3")
lines(psi[3,] ~ by, col = "blue", lwd = 2)
```
...
`r if (u_rates) "_These are total exploitation rates, with fishing mortality resulting from both subsistence and commercial fisheries. Vertical dashed lines represent the years the fisheries switched from mostly 8-inch mesh to mostly 6-inch mesh (1984 for commerical and 2014 for subsistence)._"`
...
par(mar = c(3,3,1,1), cex.main = 1.4, mgp = c(2,0.4,0), tcl = -0.2)
plot(U_fit ~ years, type = "n", ylim = c(0,1),las = 1,
     ylab = "Exploitation Rate", xlab = "Calendar Year")
...
points(x ~ years, col = "red", bg = alpha("salmon", 0.5), pch = 21)
abline(v = c(years[min(which(com_mesh == 2))], years[min(which(sub_mesh == 2))]) - 0.5, lty = 2)
...
par(mfcol = c(4,1), mar = c(2,2,2,2), oma = c(1,3,0,0), cex.main = 1.4, cex.axis = 1.4, mgp = c(2,0.4,0), tcl = -0.2)
...
mtext(side = 1, outer = T, "Calendar Year", line = 0, cex = 1.2)
...
par(mfcol = c(4,2), mar = c(2,2,2,2), oma = c(2,3,0,0), cex.main = 1.4, cex.axis = 1.4, tcl = -0.2, mgp = c(2,0.4,0))
...
par(xaxs = "i", yaxs = "i", mar = c(3,3,1,1), mgp = c(2,0.4,0), tcl = -0.2)
...
par(mar = c(3,3,1,3), mgp = c(2,0.4,0), tcl = -0.2, lend = "square")
...
x = c(rep(1, 15), rep(2, 14), rep(3, 15))
...
plot(Z_per_fS[3,] ~ years, type = "n", ylim = range(Z_per_fS[4:5,]),
     main = "", ylab = paste0(pretty_z_unit, "/Female Spawning Salmon"), xlab = "Calendar Year")
...
# lapply(param_names, function(x) match_params(post, x, type = "base_only"))
...
samps = list(
  early = post_convert(cbind(postpack:::id_mat(post), samps_early)),
  all = post_convert(cbind(postpack:::id_mat(post), samps_all)),
  late = post_convert(cbind(postpack:::id_mat(post), samps_late))
)
...
build_name = function(x) {
  ...
  m = ifelse(str_detect(x, "v_mesh8"), ",1", 
             ifelse(str_detect(x, "v_mesh6"), ",2", ""))
  group = ifelse(m == ",1", "v_mesh8", ifelse(m == ",2", "v_mesh6", group))
  data.frame(
    group = group,
...
brood_years = (min(years) - a_max):(max(years) - a_min)
...
edit_full_model(
  ...
  keep_comments = T, keep_whitespace = T, rand_age = meta$rand_age, ppd = T
  )
```
