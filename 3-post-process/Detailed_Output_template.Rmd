...
make_gof_table = function(by) {
  ...
  if (by == "age/sex") {
    colnames(tab) = c("Source", paste(paste("Age", 4:7), "Females"), paste(paste("Age", 4:7), "Males"))
  }
  if (by == "age") {
    colnames(tab) = c("Source", paste("Age", 4:7))
  }
  if (by == "total") {
    colnames(tab) = c("Source", "Total")
  }
  
  kable(tab, "html", align = paste0("l", rep("c", ncol(tab) - 1), collapse = "")) %>%
