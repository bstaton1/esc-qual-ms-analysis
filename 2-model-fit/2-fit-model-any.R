...
if (calc_EG) {
  ...
  starttime = Sys.time()
  cat("    Started at:", format(starttime), "\n")
  EG_unr_early = get_EG(post.samp = samps_early, spawn_units = ifelse(eggs, "eggs", "fish"), vuln = "unr", silent = F); cat("      Early (UNR) period complete\n")
  EG_unr_late = get_EG(post.samp = samps_late, spawn_units = ifelse(eggs, "eggs", "fish"), vuln = "unr", silent = F); cat("      Late (UNR) period complete\n")
  EG_unr_all = get_EG(post.samp = samps_all, spawn_units = ifelse(eggs, "eggs", "fish"), vuln = "unr", silent = F); cat("      All (UNR) period complete\n")
  EG_res_early = get_EG(post.samp = samps_early, spawn_units = ifelse(eggs, "eggs", "fish"), vuln = "res", silent = F); cat("      Early (RES) period complete\n")
  EG_res_late= get_EG(post.samp = samps_late, spawn_units = ifelse(eggs, "eggs", "fish"), vuln = "res", silent = F); cat("      Late (RES) period complete\n")
  EG_res_all = get_EG(post.samp = samps_all, spawn_units = ifelse(eggs, "eggs", "fish"), vuln = "res", silent = F); cat("      All (RES) period complete\n")
